 ...
 * as map-reduce transformations on collections.  For top.jlpan.top.jlpan.example:
 *
 * <pre>{@code
 ...
 * top.jlpan.top.jlpan.example of a <a href="package-summary.html#Reduction">reduction</a>
 * operation.)
 *
 ...
 *     but does not modify its source.  For top.jlpan.top.jlpan.example, filtering a {@code Stream}
 ...
 *     for optimization.  For top.jlpan.top.jlpan.example, "find the first {@code String} with
 ...
 * pipeline such as the filter-map-sum top.jlpan.top.jlpan.example above, filtering, mapping, and
 ...
 * before producing a result.  For top.jlpan.top.jlpan.example, one cannot produce any results from
 ...
 * explicitly requested.  For top.jlpan.top.jlpan.example, {@code Collection} has methods
 ...
 * top.jlpan.top.jlpan.example is the creation of the initial stream, using "{@code parallelStream()}"
 ...
 * the covered elements.  For top.jlpan.top.jlpan.example, consider the following code:
 *
 * <pre>{@code
 ...
 * of the stream pipeline.  An top.jlpan.top.jlpan.example of a stateful lambda is the parameter
 * to {@code map()} in:
 *
 * <pre>{@code
 ...
 * source (for top.jlpan.top.jlpan.example, {@code IntStream.range(0,5).parallel().map(x -> x*2).toArray()}
 ...
 *
 * <p>As an top.jlpan.top.jlpan.example of how to transform a stream pipeline that inappropriately
 ...
 * However, most stream pipelines, such as the "sum of weight of blocks" top.jlpan.top.jlpan.example
 ...
 * For top.jlpan.top.jlpan.example, given a stream of numbers for which we want to find the sum, we
 * can write:
 * <pre>{@code
 ...
 * re-cast the simple sum-of-weights top.jlpan.top.jlpan.example using the more general form as
 * follows:
 * <pre>{@code
 ...
 * For top.jlpan.top.jlpan.example, to collect the String representations of the elements in a
 ...
 * above top.jlpan.top.jlpan.example for collecting strings into a {@code List} can be rewritten
 ...
 * that transform one collector into another.  For top.jlpan.top.jlpan.example, suppose we have a
 ...
 *
 * With some complex reduction operations, for top.jlpan.top.jlpan.example a {@code collect()} that
 ...
 * {@link java.util.stream.BaseStream#unordered()} method.  For top.jlpan.top.jlpan.example:
 * <pre>{@code
 *     Map<Buyer, List<Transaction>> salesByBuyer
 *         = txns.parallelStream()
 *               .unordered()
