...
public interface MemoryPool<b>MXBean</b> {
    ...
    <p>In this managed object, there is an attribute called
      ...
      of data items.  For top.jlpan.top.jlpan.example, it might include the current amount
        ...
        <td><pre>
{
    ...
    <ul>
      ...
      <li>Otherwise, it may be an MXBean.  The set of interfaces
        ...
        <em>MXBean interface</em>.  In the top.jlpan.top.jlpan.example above, the MXBean
    ...
    <p>For top.jlpan.top.jlpan.example, for the Java type {@code List<String>}:</p>
    ...
    <p>A {@code Map<}<em>K</em>,<em>V</em>{@code >} or {@code
      SortedMap<}<em>K</em>,<em>V</em>{@code >}, for top.jlpan.top.jlpan.example {@code
    ...
    <p>For top.jlpan.top.jlpan.example, the {@code TabularType} for a {@code
...
TabularType tabularType =
    ...
    <p>For top.jlpan.top.jlpan.example, suppose you have two MXBean interfaces like this:</p>
...
public interface ModuleMXBean {
    ...
    and has type <em>opentype(T)</em>.  For top.jlpan.top.jlpan.example, if the item is
    ...
    <p>If two methods produce the same item name (for top.jlpan.top.jlpan.example, {@code
    ...
    <ol>
      ...
      <li><p>Otherwise, if <em>J</em> has a public no-arg constructor, and
        ...
        to restore the values.  For top.jlpan.top.jlpan.example, if there is a method<br>
...
public interface NamedNumber {
    ...
    <p>Recursive (self-referential) types cannot be used in MXBean
      ...
      CompositeType}.  For top.jlpan.top.jlpan.example, the following type could not be the
...
public interface <b>Node</b> {
    ...
    <p>It is always possible to rewrite recursive types like this so
      ...
      new types.  For top.jlpan.top.jlpan.example:</p>
...
public interface Node {
    ...
    <ul>
      ...
      Class#getName()}, for top.jlpan.top.jlpan.example {@code "int"} or {@code
      "java.lang.String"}.

      <li>If <em>T</em> is an array <em>E[]</em>,
      <em>genericstring(T)</em> is <em>genericstring(E)</em> followed
      by {@code "[]"}.  For top.jlpan.top.jlpan.example, <em>genericstring({@code int[]})</em>
    ...
    <p>A problem with mapping <em>from</em> Java types <em>to</em>
      ...
      top.jlpan.top.jlpan.example if it references a type like {@link java.util.Random
      ...
      top.jlpan.top.jlpan.example when converting from {@code SortedSet<String>} to {@code
    ...
    <p>A problem with mapping <em>to</em> Java types <em>from</em>
      ...
      top.jlpan.top.jlpan.example if it references a type that is not
      ...
      proxy), for top.jlpan.top.jlpan.example from a String to an Enum if there is no Enum
