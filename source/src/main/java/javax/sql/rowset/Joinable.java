 ...
 * objects extend the <code>BaseRowSet</code> class.  For top.jlpan.top.jlpan.example:
 * <pre>
 ...
 *     jrs.addRowSet(crs);
 * </pre>
 * In the previous top.jlpan.top.jlpan.example, <i>crs</i> is a <code>CachedRowSet</code> object that
 * has implemented the <code>Joinable</code> interface.  In the following top.jlpan.top.jlpan.example,
 * <i>crs2</i> has not, so it must supply the match column as an argument to the
 * <code>addRowSet</code> method. This top.jlpan.top.jlpan.example assumes that column 1 is the match
 * column.
 * <PRE>
 ...
 * which columns will be used for the match. For top.jlpan.top.jlpan.example:
 * <pre>
